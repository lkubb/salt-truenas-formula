...
We will see a quick setup to configure the ``truenas`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``truenas`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``truenas`` formulas under ``/srv/salt/truenas/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/truenas/parameters/dns:domain/
    ln -s dns:domain /srv/salt/truenas/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/truenas/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/truenas/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``truenas`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``truenas`` formulas under ``/srv/salt/truenas/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/truenas/parameters/roles
...
We create a configuration for the role ``truenas/server`` in ``/srv/salt/truenas/parameters/roles/truenas/server.yaml``:
...
We create another configuration for the role ``truenas/client`` in ``/srv/salt/truenas/parameters/roles/truenas/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles truenas/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles truenas/client
...
.. code-block:: console
    ...
    mv /srv/salt/truenas/parameters/dns:domain/  '/srv/salt/truenas/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-truenas-as-json:
      test.nop:
        - name: {{ truenas | json }}
...
Here is an example based on `template-formula/truenas/config/file.sls`_:
    ...
    truenas-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='truenas-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ truenas.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``truenas`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ truenas['winner'] }}
...
.. _template-formula/truenas/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/truenas/config/file.sls
